...
/// Generate the tx data to mint SUI_USDE tokens using USDC collateral.
const mintData = async () => {
  ...
  if (!address || !amountArg || !expiryMsArg || !minAmountOutArg || !nonce) {
    console.error(
      "Usage: mint <sender_address> <amount_usdc> <expiry_minutes> <min_amount_out> <nonce>",
    );
    console.error(
      "  amount_usdc:     Amount of USDC to mint (e.g., 100 for 100 USDC)",
    );
    console.error(
      "  expiry_minutes:  Order expiry in minutes from now (e.g., 5 for 5 minutes)",
    );
    console.error(
      "  min_amount_out:  Minimum output amount (6 decimals, e.g., 99.5 for 99.5 tokens)",
    );
