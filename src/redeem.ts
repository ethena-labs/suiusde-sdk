...
const redeemData = async () => {
  ...
  if (!address || !amountArg || !expiryMsArg || !minAmountOutArg || !nonce) {
    console.error(
      "Usage: redeem <sender_address> <amount_suiusde> <expiry_minutes> <min_amount_out> <nonce>",
    );
    console.error(
      "  amount_suiusde:  Amount of SUI_USDE to redeem (e.g., 100 for 100 tokens)",
    );
    console.error(
      "  expiry_minutes:  Order expiry in minutes from now (e.g., 5 for 5 minutes)",
    );
    console.error(
      "  min_amount_out:  Minimum USDC output (6 decimals, e.g., 99.5 for 99.5 USDC)",
    );
